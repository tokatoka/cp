...
signed main(){
  ...
  double ans = 0.0;
  double prev = 0.0;
  for(double i = 0.5 ; i<= sum_t[n]; i += 0.5){
    double cur = 100;
    for(int j = 0 ; j <= n + 1; j++){
      if(j >= 1 && sum_t[j-1] <= i && i <= sum_t[j]){
        cur = min(cur,(double)v[j]);
      }
      if(sum_t[j] < i){
        cur = min(cur,(double)(v[j] + (i - sum_t[j])));
      }
      if(j >= 1 && sum_t[j-1] > i){
        cur = min(cur,(double)(v[j] + (sum_t[j-1] - i)));
      }
    }
    ans += (cur+prev) * 0.25;
    prev = cur;
