  ...
  for(int i = 0 ; i <= 30 ; i++){
    for(int j = 0 ; j <= 30 ; j++){
      if(i * A * 100 + j * B * 100 <= F){
        ...
        //max sugar : 30 * E / C
        for(int a = 0 ; a <= 30 * E ; a++){
          for(int b = 0 ; b <= 30 * E ; b++){
            if(water + a * C + b * D <= F){
              int sugar = a * C + b * D;
              if(sugar <= (double)water*(double)E/(double)100){
                //passed all regulation
                double d = (double)(100 * sugar)/((double)water + (double)sugar);
                if(d > max_d){
                  max_d = d;
                  max_water = water;max_sugar = sugar;
                }
              }
            }
          }
